    ...
    async def update_analysis_record(
        ...
        try:
            async with async_session_maker() as session:
                ...
                analysis.analysis = analysis_data.get(
                    "analysis") or ""  # Ensure analysis is never None
                analysis.key_points = analysis_data.get("key_points", [])
                analysis.conflicts = analysis_data.get("conflicts", [])
                analysis.recommendations = analysis_data.get(
                    "recommendations", [])
                analysis.chunk_analyses = analysis_data.get(
                    "chunk_analyses", [])
                analysis.total_chunks_analyzed = analysis_data.get(
                    "total_chunks_analyzed", 0)
                analysis.processing_time_seconds = analysis_data.get(
                ...
                await session.commit()
                logger.info(
                    f"Updated analysis record {analysis_id} with results")
    ...
    async def update_analysis_record_by_fp(
        ...
        try:
            async with async_session_maker() as session:
                ...
                await session.commit()
                logger.info(
                    f"Updated analysis record with fp {analysis_fp} with results"
                )
