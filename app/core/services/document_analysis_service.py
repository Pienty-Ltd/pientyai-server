...
class DocumentAnalysisService:
    ...
    async def analyze_document_with_knowledge_base(
        ...
        user_id: int,
        max_relevant_chunks:
        int = 15,  # Increased from 5 to 15 to get more KB chunks
        ...
        try:
            ...
            if analysis_id:
                ...
                if not analysis_record:
                    logger.warning(
                        f"Analysis ID {analysis_id} provided but record not found, creating new record"
                    )
            ...
            logger.info(
                f"Finding KB chunks for all {total_chunks} document chunks using enhanced retrieval"
            )

            for chunk_idx, chunk in enumerate(document_chunks):
                logger.info(
                    f"Processing document chunk {chunk_idx+1}/{total_chunks}")

                # Skip empty chunks
                if not chunk.content or not chunk.content.strip():
                    logger.debug(
                        f"Skipping empty document chunk at index {chunk_idx}")
                    continue
            ...
            logger.info(
                "Finding KB chunks based on the entire document content")
            ...
            try:
                logger.info(
                    f"Retrieving KB chunks for the full document content")
                ...
                full_doc_relevant_kb = await self.find_relevant_knowledge_base_chunks(
                    ...
                    current_document_id=document_id,
                    limit=
                    10  # Tam belge için komple context'i koruma adına daha yüksek limit kullanılabilir
                )
                ...
                logger.info(
                    f"Added {len(full_doc_relevant_kb)} KB chunks from full document analysis"
                )
            except Exception as e:
                logger.error(
                    f"Error retrieving KB chunks for full document: {str(e)}")
            ...
            logger.info(
                f"Collected {len(all_kb_chunks)} unique knowledge base chunks")
            ...
            logger.info(
                f"Knowledge base chunks distribution by document: {doc_count}")
            ...
            for doc_id, chunks in document_chunks.items():
                ...
                for kb_info, score in chunks:
                    ...
                    # Sadece tam olarak aynı index'i kontrol et (daha az agresif)
                    # Bu, daha fazla KB chunk'ın dahil edilmesini sağlar
                    exact_duplicate = chunk_idx in selected_indexes

                    # Eğer tam olarak aynı chunk zaten seçilmişse, atla
                    should_add = not exact_duplicate

                    # 24 chunk olan bir dokümanda daha fazla KB chunk kullanmak için
                    # içerik benzerliği filtrelemesini tamamen kaldırıyoruz.
                    # Burada amaç, her doküman chunk'ı için en benzer KB chunklarını ve komşularını
                    # mümkün olduğunca fazla korumaktır.

                    # İçerik benzerliği kontrolünü çok yüksek bir değere ayarlayarak
                    # neredeyse tamamen devre dışı bırakıyoruz (sadece %97 benzerlik olanları filtreler)
                    if should_add and len(deduplicated_chunks) > 0:
                        # Sadece tam olarak aynı içeriğe sahip olanları filtrele
                        # Önceki gibi, son birkaç eklenen chunk ile karşılaştır
                        for i in range(max(0,
                                           len(deduplicated_chunks) - 2),
                                       len(deduplicated_chunks)):
                            previous_chunk = deduplicated_chunks[i][0]

                            # Tam olarak aynı içerik varsa (duplicate içerik) filtrele
                            if (previous_chunk["document_id"]
                                    == kb_info["document_id"]
                                    and previous_chunk["content"]
                                    == kb_info["content"]):
                                should_add = False
                                logger.debug(
                                    f"Skipping exact duplicate content from {kb_info['document_name']} index {chunk_idx}"
                                )
                                break
                    ...
                    if should_add:
                        ...
                        logger.debug(
                            f"Skipping nearby chunk {kb_info['document_name']} index {chunk_idx} (score: {score:.4f})"
                        )

                # Log how many chunks were selected from this document
                logger.debug(
                    f"Selected {len(selected_indexes)} chunks from document ID {doc_id} (from {len(chunks)} candidates)"
                )
            ...
            logger.info(
                f"Selected chunks distribution after deduplication: {selected_doc_counts}"
            )
            ...
            logger.info(
                f"Using all {len(deduplicated_chunks)} deduplicated chunks from a total of {len(kb_chunks_with_metadata)}"
            )
            ...
            logger.info(
                f"Prepared {len(formatted_kb_chunks)} KB chunks for OpenAI analysis"
            )
            ...
            logger.info(
                f"Preparing OpenAI analysis with {len(formatted_kb_chunks)} knowledge base chunks..."
            )

            # Knowledge base'in boş olması durumunu kontrol et
            if len(formatted_kb_chunks) == 0:
                logger.warning(
                    "No knowledge base chunks found for analysis - this may impact analysis quality"
                )
                ...
                try:
                    async with async_session_maker() as session:
                        ...
                        kb_count_query = select(
                            func.count()).select_from(KnowledgeBase).where(
                        ...
                        logger.info(
                            f"Organization has {kb_count} total knowledge base entries"
                        )

                        if kb_count == 0:
                            logger.warning(
                                f"Organization {organization_id} has no knowledge base entries"
                            )
                        else:
                            # Neden bulunamadığını araştır
                            logger.warning(
                                f"Organization has knowledge base entries but none were relevant for this document"
                            )
                except Exception as kb_check_err:
                    logger.error(
                        f"Error checking knowledge base status: {str(kb_check_err)}"
                    )
            ...
            try:
                ...
                if analysis_result is None or not isinstance(
                        analysis_result, dict):
                    logger.error(
                        f"Invalid analysis result returned: {analysis_result}")
                ...
                if "diff_changes" not in analysis_result:
                    logger.warning(
                        "Analysis result does not contain diff_changes, adding empty value"
                    )
                    analysis_result[
                        "diff_changes"] = "No changes identified in the document."

                analysis_duration = (datetime.now() -
                                     analysis_start_time).total_seconds()
                total_processing_time += analysis_duration

                logger.info(
                    f"Successfully completed document analysis in {analysis_duration:.2f} seconds"
                )

                # Add additional metadata
                analysis_result["processing_time_seconds"] = analysis_duration
                analysis_result["total_chunks_analyzed"] = len(
                    formatted_kb_chunks)
                ...
                analysis_response = {
                    ...
                    "document_info": {
                        ...
                        "name":
                        document.filename
                        if hasattr(document, 'filename') else "Unknown",
                    }
                }
                ...
                try:
                    ...
                    logger.info(
                        f"Analysis record updated successfully for ID: {analysis_record.id}"
                    )
                except Exception as record_error:
                    logger.error(
                        f"Failed to update analysis record: {str(record_error)}"
                    )
                    # Hata olsa bile işlemi devam ettir, kullanıcıya yanıt döndür

                logger.info(
                    f"Document analysis completed successfully in {total_processing_time:.2f} seconds"
                )
                ...
                try:
                    ...
                    basic_response = {
                        "diff_changes":
                        f"Analysis could not be completed due to an error: {str(e)}. Please try again later.",
                        "processing_time_seconds":
                        (datetime.now() - analysis_start_time).total_seconds(),
                        "total_chunks_analyzed":
                        len(formatted_kb_chunks),
                        "error_details":
                        str(e)
                    }
                    ...
                    await self.update_analysis_record(
                        ...
                        COMPLETED  # Önemli: FAILED yerine COMPLETED kullan
                    )

                    logger.info(
                        "Returning partial analysis response despite error")
                    return basic_response

                except Exception as update_error:
                    logger.error(
                        f"Failed to update analysis with error info: {str(update_error)}"
                    )
    ...
    async def find_relevant_knowledge_base_chunks(
        ...
        Retrieves multiple similar KB chunks along with their adjacent chunks (one before and one after each).
        This provides better context and ensures adequate coverage for each document chunk.
        ...
        Args:
            ...
            limit: Maximum number of chunks to return (between 3 and 20)
        ...
        """
        try:
            # Ensure limit is within reasonable bounds - larger limit to ensure better coverage
            # We're now getting more matching chunks per document chunk
            ...
            # Use vector similarity search to find multiple similar chunks
            async with async_session_maker() as session:
                from sqlalchemy import text
                embedding_string = json.dumps(query_embedding)

                # Get more similar chunks initially - we'll add their neighbors later
                # Burada belge başına daha fazla KB chunk alıyoruz (en az 3 adet, limit/3 kadar)
                initial_limit = max(3, min(
                    limit // 3,
                    5))  # Get more chunks initially - at least 3, at most 5

                # SQL query to find multiple similar KB chunks
                sql_query = text(f"""
                    SELECT
                        *,
                ...
                if not similar_rows:
                    logger.warning(
                        f"No similar KB chunks found for query in organization {organization_id}"
                    )
                    return []

                # Create KB objects for the similar chunks
                # Map to track unique chunks by fingerprint
                fp_to_chunks = {}
                result_chunks = []

                # Process each similar chunk and add its neighbors
                for row in similar_rows:
                    # Create KB object for this similar chunk
                    ...
                    setattr(kb_chunk, 'similarity_score', row.similarity_score)

                    # Add to our collection if not already there
                    if kb_chunk.fp not in fp_to_chunks:
                        fp_to_chunks[kb_chunk.fp] = kb_chunk
                        result_chunks.append(kb_chunk)

                    # Find and add adjacent chunks (previous and next)
                    document_id = kb_chunk.file_id
                    chunk_index = kb_chunk.chunk_index
                    ...
                    if chunk_index > 0:
                        ...
                        if prev_chunk and prev_chunk.fp not in fp_to_chunks:
                            # Set a synthetic similarity score slightly lower than the similar chunk
                            setattr(
                                prev_chunk, 'similarity_score',
                                getattr(kb_chunk, 'similarity_score', 0) +
                                0.01)
                            fp_to_chunks[prev_chunk.fp] = prev_chunk
                    ...
                    if next_chunk and next_chunk.fp not in fp_to_chunks:
                        # Set a synthetic similarity score slightly lower than the similar chunk
                        setattr(
                            next_chunk, 'similarity_score',
                            getattr(kb_chunk, 'similarity_score', 0) + 0.02)
                        fp_to_chunks[next_chunk.fp] = next_chunk
                        result_chunks.append(next_chunk)

                # Group chunks by document ID and sort by chunk index to maintain document flow
                # This helps maintain context within each document
                grouped_chunks = {}
                ...
                for doc_id, chunks in grouped_chunks.items():
                    # Sort this document's chunks by index
                    chunks.sort(key=lambda x: x.chunk_index)
                    # Add to final result
                    sorted_chunks.extend(chunks)

                # For consistency with rest of code, sort by similarity score
                # But maintain document grouping
                sorted_chunks.sort(
                    key=lambda x: getattr(x, 'similarity_score', 0))

                # Limit total chunks if we have too many
                if len(sorted_chunks) > limit:
                    sorted_chunks = sorted_chunks[:limit]

                logger.info(
                    f"Found {len(similar_rows)} similar chunks with {len(sorted_chunks)} total chunks (including adjacent)"
                )
            ...
            if "vector" in error_str or "cosine" in error_str or "embedding" in error_str:
                logger.critical(
                    "Possible pgvector extension issue or embedding format problem"
                )
            ...
            try:
                ...
                logger.info(
                    "Attempting fallback query to get recent knowledge base entries"
                )
                async with async_session_maker() as session:
                    fallback_query = select(KnowledgeBase).where(
                        KnowledgeBase.organization_id == organization_id,
                        KnowledgeBase.is_knowledge_base == True).order_by(
                            desc(KnowledgeBase.created_at)).limit(limit)
                    ...
                    if fallback_chunks:
                        logger.info(
                            f"Retrieved {len(fallback_chunks)} recent knowledge base chunks as fallback"
                        )
                        return fallback_chunks
                    else:
                        logger.warning(
                            f"No knowledge base entries found for organization {organization_id}"
                        )
            except Exception as fallback_error:
                logger.error(
                    f"Fallback query also failed: {str(fallback_error)}")
            ...
            async with async_session_maker() as session:
                ...
                if analysis:
                    ...
                    if "total_chunks_analyzed" in analysis_data:
                        analysis.total_chunks_analyzed = analysis_data.get(
