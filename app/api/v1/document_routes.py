...
from app.api.v1.middlewares.validation_middleware import (
    validate_pagination_parameters, validate_document_fp,
    validate_organization_id, validate_organization_fp)
    ...
    try:
        ...
        if file_ext not in EXTENSION_MIME_MAP:
            raise HTTPException(
                ...
                detail=
                f"Unsupported file type. Allowed types: {', '.join(ALLOWED_MIMETYPES.values())}"
            )
        ...
        if content_type not in ALLOWED_MIMETYPES:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=
                f"Invalid content type. Expected types: {', '.join(ALLOWED_MIMETYPES.keys())}"
            )
        ...
        if file_ext != expected_ext:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=
                f"File extension '{file_ext}' doesn't match content type. Expected: {expected_ext}"
            )
        ...
        if content_type != expected_mime:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=
                f"Content type mismatch. Got: {content_type}, Expected: {expected_mime}"
            )
...
async def process_document_background(file_content: bytes,
                                      filename: str,
                                      file_type: str,
                                      user_id: int,
                                      organization_id: int,
    ...
    try:
        logger.info(
            f"Starting background processing for document {filename} (ID: {db_file_id}, knowledge_base: {is_knowledge_base})"
        )
        ...
        logger.info(
            f"Completed background processing for document {filename} (Duration: {processing_duration}s)"
        )
    except Exception as e:
        ...
        document_service = DocumentService()
        await document_service.update_file_status(db_file_id,
                                                  FileStatus.FAILED)
...
async def list_user_documents(
    ...
    try:
        ...
        validated_page, validated_per_page = await validate_pagination_parameters(
        ...
        if organization_fp:
            ...
            logger.info(
                f"Fetching documents for user {current_user.id} in organization {organization_fp}, page {page}, per_page {per_page}"
            )
        else:
            logger.info(
                f"Fetching documents for user {current_user.id} across all organizations, page {page}, per_page {per_page}"
            )
        ...
        logger.info(
            f"Retrieved {len(documents)} documents (Total: {total_count}, Duration: {duration}s)"
        )
...
@router.get("/organization/{org_fp}",
            response_model=BaseResponse[PaginatedDocumentResponse])
async def list_organization_documents(
    ...
    try:
        ...
        logger.info(
            f"Fetching documents for organization {org_fp}, page {page}, per_page {per_page}"
        )
        start_time = datetime.now()
        ...
        if not any(org.id == organization.id
                   for org in current_user.organizations):
            logger.warning(
                f"Access denied: User {current_user.id} attempted to access org {org_fp}"
            )
        ...
        duration = (datetime.now() - start_time).total_seconds()
        logger.info(
            f"Retrieved {len(documents)} documents for org {org_fp} (Total: {total_count}, Duration: {duration}s)"
        )
...
@router.get("/document/{document_fp}/chunks",
            response_model=BaseResponse[PaginatedKnowledgeBaseResponse])
async def get_document_chunks(document_fp: str,
                              page: int = Query(1,
                                                gt=0,
    ...
    try:
        logger.info(
            f"Fetching chunks for document {document_fp}, page={page}, per_page={per_page}"
        )
        start_time = datetime.now()
        ...
        document, organization = await document_service.get_document_by_fp_for_user(
            user_id=current_user.id, document_fp=document_fp)

        if not document or not organization:
            logger.warning(
                f"Document not found or access denied: FP {document_fp}")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail="Document not found")

        # Get chunks with pagination
        chunks, total_count = await document_service.get_document_chunks(
            document_id=document.id, page=page, per_page=per_page)

        # Calculate pagination info
        total_pages = math.ceil(total_count /
                                per_page) if total_count > 0 else 0

        duration = (datetime.now() - start_time).total_seconds()
        logger.info(
            f"Retrieved {len(chunks)} chunks for document {document_fp} (Total: {total_count}, Duration: {duration}s)"
        )

        # Convert KnowledgeBase models to KnowledgeBaseResponse objects
        knowledge_base_responses = [
            KnowledgeBaseResponse(fp=chunk.fp,
...
@router.get("/document/{document_fp}/search",
            response_model=BaseResponse[PaginatedKnowledgeBaseResponse])
async def search_document_chunks(
    ...
    try:
        logger.info(
            f"Searching chunks in document {document_fp} with query '{query}', page={page}, per_page={per_page}"
        )
        start_time = datetime.now()
        ...
        chunks, total_count, document, organization = await document_service.search_chunks_in_document_by_fp(
            document_fp=document_fp, query=query, page=page, per_page=per_page)
        ...
        if not any(org.id == organization.id
                   for org in current_user.organizations):
            logger.warning(
                f"Access denied: User {current_user.id} attempted to search chunks in document {document_fp}"
            )
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
        ...
        duration = (datetime.now() - start_time).total_seconds()
        logger.info(
            f"Found {len(chunks)} chunks in document {document_fp} matching query (Total: {total_count}, Duration: {duration}s)"
        )
...
@router.get("/document/{document_fp}",
            response_model=BaseResponse[DocumentWithChunksResponse])
async def get_document(
    document_fp: str,
    include_chunks: bool = Query(
        True,
        description="Whether to include document chunks in the response"),
    current_user: User = Depends(get_current_user)):
    """
    ...
    try:
        logger.info(
            f"Fetching document {document_fp}, include_chunks={include_chunks}"
        )
        start_time = datetime.now()
        ...
        duration = (datetime.now() - start_time).total_seconds()
        logger.info(
            f"Retrieved document {document_fp} with {len(response.chunks)} chunks (Duration: {duration}s)"
        )
...
async def delete_document(document_fp: str,
                          current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        if not success:
            logger.warning(
                f"Document not found for deletion: FP {document_fp}")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail="Document not found")

        duration = (datetime.now() - start_time).total_seconds()
        logger.info(
            f"Deleted document {document_fp} successfully (Duration: {duration}s)"
        )
...
async def upload_document(
    ...
    try:
        logger.info(
            f"Processing upload request for file {file.filename} to organization {organization_fp} (knowledge_base: {is_knowledge_base})"
        )
        start_time = datetime.now()
        ...
        # Fetch organization by fingerprint
        document_service = DocumentService()
        organization = await document_service.get_organization_by_fp(
            organization_fp)
        ...
        if not any(org.id == organization.id
                   for org in current_user.organizations):
            logger.warning(
                f"Access denied: User {current_user.id} attempted to upload to org {organization_fp}"
            )
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="Access denied to organization")
        ...
        duration = (datetime.now() - start_time).total_seconds()
        logger.info(
            f"Document upload initiated for {file.filename} (Duration: {duration}s)"
        )
...
@router.get("/search/{org_fp}",
            response_model=BaseResponse[PaginatedDocumentResponse])
async def search_documents(org_fp: str,
    ...
    try:
        logger.info(
            f"Searching documents in organization {org_fp} with query '{query}', page {page}, per_page {per_page}"
        )
        start_time = datetime.now()
        ...
        if not any(org.id == organization.id
                   for org in current_user.organizations):
            logger.warning(
                f"Access denied: User {current_user.id} attempted to search in org {org_fp}"
            )
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="Access denied to organization")
        ...
        duration = (datetime.now() - start_time).total_seconds()
        logger.info(
            f"Organization document search completed (Duration: {duration}s), {len(documents)} results found, {total_count} total."
        )
...
async def search_user_documents(
    ...
    try:
        logger.info(
            f"Searching all user accessible documents with query '{query}', page {page}, per_page {per_page}"
        )
        start_time = datetime.now()
        ...
        results, total_count = await document_service.search_documents_for_user(
            user_id=current_user.id, query=query, page=page, per_page=per_page)
        ...
        duration = (datetime.now() - start_time).total_seconds()
        logger.info(
            f"User document search completed (Duration: {duration}s), {len(results)} results found, {total_count} total."
        )

        # Convert File models to DocumentResponse objects
        document_responses = [
            DocumentResponse(fp=file.fp,
                             filename=file.filename,
...
@router.get("/search/chunks/{org_fp}",
            response_model=BaseResponse[PaginatedKnowledgeBaseResponse])
async def search_organization_document_chunks(
    ...
    try:
        logger.info(
            f"Searching document chunks in organization {org_fp} with query '{query}', page {page}, per_page {per_page}"
        )
        start_time = datetime.now()
        ...
        if not any(org.id == organization.id
                   for org in current_user.organizations):
            logger.warning(
                f"Access denied: User {current_user.id} attempted to search chunks in org {org_fp}"
            )
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="Access denied to organization")
        ...
        duration = (datetime.now() - start_time).total_seconds()
        logger.info(
            f"Organization chunk search completed (Duration: {duration}s), {len(results)} chunks found, {total_count} total."
        )
        ...
        logger.error(
            f"Error searching document chunks in organization: {str(e)}",
            exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An error occurred while searching document chunks")
...
async def search_user_document_chunks(
    ...
    try:
        logger.info(
            f"Searching document chunks with query '{query}', page {page}, per_page {per_page}"
        )
        start_time = datetime.now()
        ...
        duration = (datetime.now() - start_time).total_seconds()
        logger.info(
            f"Chunk search completed (Duration: {duration}s), {len(results)} chunks found, {total_count} total."
        )
